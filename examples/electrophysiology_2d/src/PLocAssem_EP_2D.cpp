...
PLocAssem_EP_2D::PLocAssem_EP_2D(
...
void PLocAssem_EP_2D::Zero_Tangent_Residual()
{
  for(int ii=0; ii<vec_size; ++ii)
    Residual[ii] = 0.0;
  for(int ii=0; ii<vec_size * vec_size; ++ii)
...
void PLocAssem_EP_2D::Zero_Residual()
{
  for(int ii=0; ii<vec_size; ++ii)
    Residual[ii] = 0.0;
}
...
void PLocAssem_EP_2D::Assem_Estimate()
{
  for(int ii=0; ii<vec_size * vec_size; ++ii)
    Tangent[ii] = 1.0;
}
...
void PLocAssem_EP_2D::Assem_Mass(
    const class FEAElement * const &element,
    const class AInt_Weight * const &weight )
{
...
void PLocAssem_EP_2D::Assem_Mass_Residual(
    const double * const &disp,
    //const double * const &Iion,
    //const double * const &dPhi_Iion,    
    const class FEAElement * const &element,
