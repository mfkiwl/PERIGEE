...
IonicModel_TTPPurkinje::IonicModel_TTPPurkinje(const double &cond_scale,
			       const double &delay_in)
  //           d_iso,     d_ani,     chi,   C_m, n_int_vars
  : IonicModel( cond_scale*0.66172,  0.0, 140.0, 0.01, 18),
    ...
    K_o{5.4e0}, Na_o{140.0e0}, Ca_o{2.0e0}, G_Na{43.62372e0},
    G_K1{5.405e0}, G_to{0.073e0, 0.294e0, 0.294e0}, G_Kr{0.153e0},
    G_Ks{0.1372e0, 0.1372e0, 0.0343e0}, p_KNa{0.03e0}, G_CaL{3.98e-5},
...
IonicModel_TTPPurkinje::~IonicModel_TTPPurkinje()
{
  //SYS_T::commPrint("AP destructor. \n");
};

void IonicModel_TTPPurkinje::print_info () const
{
  PetscPrintf(PETSC_COMM_WORLD, "\t  Ten-Tusscher Panfilov EP model for Purkinje \n");
  //PetscPrintf(PETSC_COMM_WORLD, "\t  Rc = %.2e \n", Rc);
  //PetscPrintf(PETSC_COMM_WORLD, "\t  G_to = [%.2e, %.2e, %.2e] \n",
  //                  G_to[0], G_to[1],G_to[2] ); 
};

void IonicModel_TTPPurkinje::run_ionic(const std::vector<double> &r_old_in,
			       ...
			       double &V_new) const
{
  ...
  if ((i==0)||(i==2)) {
    ...
    SYS_T::print_exit("IonicModel_TTPPurkinje: i should be  1, 2 or 3");
  }
...
void IonicModel_TTPPurkinje::get_Istim(double &Istim,
			       const double &t,
			       const double &x,
			       const double &y,
...
void IonicModel_TTPPurkinje::get_int_vars(double* val) const
{
