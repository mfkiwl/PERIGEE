...
IonicModel_Passive::IonicModel_Passive()
  //           d_iso,   d_ani, chi,   C_m  n_int_var
  : IonicModel(5.0,     0.0,   140.0, 0.001, 1)
    // ap_1{80.0}, ap_2{80.0}, ap_3{3.0}, m1{0.2},
    // m2{0.3}, alpha{0.01}, gamma{0.002}, b{0.15} , c{8.0} 
{
  //SYS_T::commPrint("IonicModel_Passive constructor. \n");
};

IonicModel_Passive::~IonicModel_Passive()
{
  //SYS_T::commPrint("IonicModel_Passive destructor. \n");
};

void IonicModel_Passive::print_info () const
{
  PetscPrintf(PETSC_COMM_WORLD, "\t  Passive ionic model\n");
  PetscPrintf(PETSC_COMM_WORLD, "\t  d_iso = %e \n", d_iso);
  // PetscPrintf(PETSC_COMM_WORLD, "\t  ap_2 = %e \n", ap_2);
  // PetscPrintf(PETSC_COMM_WORLD, "\t  ap_3 = %e \n", ap_3);
};

void IonicModel_Passive::get_Istim(double &Istim,
				    const double &t,
				    const double &x,
				    const double &y,
...
void IonicModel_Passive::run_ionic(const std::vector<double> &r_old_in,
				    ...
				    double &V_new) const
{
  ...
  V_new = V_old_in -  I_stim/chi * dt_in/C_m;
  r_new = r_old_in;
}


void IonicModel_Passive::get_int_vars(double* val) const
{
  //SYS_T::commPrint("Passive ionic,  get int vars. \n"); 
  val[ 0 ]=  0 ;
};
