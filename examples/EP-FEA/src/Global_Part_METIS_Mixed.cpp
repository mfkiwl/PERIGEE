...
Global_Part_METIS_Mixed::Global_Part_METIS_Mixed( const int &cpu_size,
...
: isMETIS(true), isDual(isDualGraph), dual_edge_ncommon(in_ncommon)
{
  ...
  if(isDualGraph)
  {
    if( in_ncommon < 1 || in_ncommon > nLocBas_tmp - 1 )
    {
  ...
  const idx_t eind_size = mesh->get_nElemXnLocBas(); 
  ...
  // insides of for loop  if  ee=0
  eptr[0] = 0;
  for(int ii=0; ii<(mesh->get_nLocBas(0)); ++ii){
    eind[ii] = IEN->get_IEN(0, ii);
  }

  for( idx_t ee=1; ee<nElem; ++ee )
  {
    eptr[ee] = eptr[ee-1] + mesh->get_nLocBas(ee-1);
    for(int ii=0; ii<(mesh->get_nLocBas(ee)); ++ii)
      eind[eptr[ee] + ii] = IEN->get_IEN(ee, ii);
  }
  eptr[nElem] = mesh->get_nElemXnLocBas();
...
Global_Part_METIS_Mixed::~Global_Part_METIS_Mixed()
{
  delete [] epart; delete [] npart;
  epart = nullptr; npart = nullptr;
  std::cout<<"-- Global_Part_METIS_Mixed deleted. \n";
}

void Global_Part_METIS_Mixed::write_part_hdf5( const char * const &fileName,
...
void Global_Part_METIS_Mixed::write_part_hdf5_64bit( const char * const &fileName,
