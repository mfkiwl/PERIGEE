...
void Part_Mixed_Mesh::Generate_Partition( const IMesh * const &mesh,
    const IGlobal_Part * const &gpart,
    const Map_Node_Index * const &mnindex,
    const IIEN * const &IEN,
    const std::vector<double> &ctrlPts,
    const bool &isPrintInfo )
{
  ...
  for( int e=0; e<nlocalele; ++e )  {
    nLocBas_loc.at(e) = nLocBas.at(elem_loc[e]);
  ...
  for(int e=0; e<nlocalele; ++e)
    LIEN[e] = new int [nLocBas_loc.at(e)];
  
  std::vector<int>::iterator lien_ptr;
  int global_index;
  for(int e=0; e<nlocalele; ++e)
  {
    for(int i=0; i<(nLocBas_loc.at(e)); ++i)
    {
...
int Part_Mixed_Mesh::get_elemLocIndex(const int &gloindex) const
{
...
void Part_Mixed_Mesh::write( const char * inputFileName ) const
{
  ...
  std::vector<int> row_stu_degree;
  row_stu_degree.resize(nElem * probDim);
  for(int e=0; e<nElem; ++e)  {
  ...
  h5w->write_intVector( group_id_3, "elemType", elemType );
  ...
  //row_LIEN.resize(nElemXnLocBas_loc);
  for(int e=0; e<nlocalele; ++e)  {
    for(int ii=0; ii<(nLocBas_loc.at(e)); ++ii)
...
void Part_Mixed_Mesh::print_part_ele() const
{
...
void Part_Mixed_Mesh::print_part_node() const
{
...
void Part_Mixed_Mesh::print_part_ghost_node() const
{
...
void Part_Mixed_Mesh::print_part_local_to_global() const
{
...
void Part_Mixed_Mesh::print_part_LIEN(const IMesh * const &mesh) const
{
  std::cout<<"Proc: "<<cpu_rank<<" LIEN: "<<std::endl;
  for(int ee=0; ee<nlocalele; ++ee)
  {
    for(int ii=0; ii<(nLocBas_loc.at(ee)); ++ii)
      std::cout<<LIEN[ee][ii]<<'\t';
    std::cout<<std::endl;
  }
  std::cout<<std::endl;
...
void Part_Mixed_Mesh::print_part_loadbalance_edgecut() const
{
  std::cout<<"Proc:"<<" "<<cpu_rank;
